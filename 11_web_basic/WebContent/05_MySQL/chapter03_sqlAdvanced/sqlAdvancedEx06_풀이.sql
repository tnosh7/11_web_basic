...
SELECT	*
FROM	PRODUCT_TB
WHERE	PRODUCT_CD IN (SELECT PRODUCT_CD FROM ORDER_TB WHERE ORDER_GOODS_QTY >= 10); 
...
SELECT *
FROM 	ORDER_TB
WHERE 	ORDER_GOODS_QTY = (SELECT MAX(ORDER_GOODS_QTY) FROM ORDER_TB);                      
...
WHERE 	PRODUCT_CD = (SELECT PRODUCT_CD 
						FROM ( SELECT  MAX(ORDER_GOODS_QTY)
								FROM 	ORDER_TB) AS ORDER_GOODS_QTY);
